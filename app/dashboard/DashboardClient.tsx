...
function formatQtyLabelBubble(qty: number, unit: string) {
  const u = String(unit ?? "").trim() || "Und";
  const q = Number.isFinite(qty) ? qty : 0;
  const label = q === 0 ? "0" : q.toLocaleString("pt-BR", { minimumFractionDigits: 0, maximumFractionDigits: 3 });
  return `${label} ${u}`.trim();
}
...
function isPrePreparoInventoryId(value: string) {
  return String(value ?? "").startsWith("prep:");
}

function extractPrePreparoIdFromInventoryId(value: string) {
  ...
  function handleCalculate() {
    ...
    const prePreparoItems = safeArray<any>(prePreparo).map((r) => {
      ...
      const list = [
        ...insumos
          .filter((i) => !i.ocultar)
          .map((i) => ({ kind: "insumo" as const, id: i.id, item: i.item, categoria: i.categoria ?? "-", medida: i.medida || "Und", custoInicial: String(i.custoMedio ?? "") })),
