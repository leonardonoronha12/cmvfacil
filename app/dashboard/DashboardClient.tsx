...
function buildSampleFornecedorSeed(insumosRows: InsumoStoreItem[]) {
  const baseFornecedores = [
    {
      key: "WMS SUPERMERCADOS DO BRASIL LTDA",
      fornecedor: "WMS Supermercados do Brasil Ltda",
      vendedor: "-",
      whatsapp: "",
      endereco: "-",
    },
    {
      key: "ATACADAO DISTRIBUICAO COMERCIO E INDUSTRIA LTDA",
      fornecedor: "Atacadao Distribuicao Comercio e Industria Ltda",
      vendedor: "-",
      whatsapp: "",
      endereco: "-",
    },
    {
      key: "MARTINS COMERCIO E SERVICOS DE DISTRIBUICAO SA",
      fornecedor: "Martins Comercio e Servicos de Distribuicao SA",
      vendedor: "-",
      whatsapp: "",
      endereco: "-",
    },
  ...
  const itemNames = insumosRows
    .map((row) => row.item.trim())
    .filter(Boolean)
    .filter((name, index, arr) => arr.findIndex((x) => x.toLowerCase() === name.toLowerCase()) === index);

  const info: FornecedorInfoMap = {};
  const produtos: FornecedorProdutos = {};
  const equivalencias: FornecedorEquivalenciasMap = {};

  for (const fornecedor of baseFornecedores) {
    info[fornecedor.key] = {
    ...
    produtos[fornecedor.key] = [];
    equivalencias[fornecedor.key] = [];
  }

  itemNames.forEach((item, index) => {
    const fornecedor = baseFornecedores[index % baseFornecedores.length];
    produtos[fornecedor.key] = [...(produtos[fornecedor.key] ?? []), item];
    equivalencias[fornecedor.key] = [
      ...(equivalencias[fornecedor.key] ?? []),
      {
        id: `${fornecedor.key}-${index + 1}`,
        nomeNaNota: item,
        unidadeNaNota: "Und",
        insumoEquivalente: item,
        equivalenteQuantidade: "1",
        equivalenteUnidade: "Und",
      },
    ];
  });

  return { info, produtos, equivalencias };
}

function mergeSampleFornecedorSeed(
  insumosRows: InsumoStoreItem[],
  infoRows: FornecedorInfoMap,
  produtosRows: FornecedorProdutos,
  equivalenciasRows: FornecedorEquivalenciasMap,
) {
  const seed = buildSampleFornecedorSeed(insumosRows);
  const nextInfo: FornecedorInfoMap = { ...infoRows };
  const nextProdutos: FornecedorProdutos = { ...produtosRows };
  const nextEquivalencias: FornecedorEquivalenciasMap = { ...equivalenciasRows };
  const coveredItems = new Set<string>();
  let changed = false;

  for (const items of Object.values(produtosRows)) {
    for (const item of items) coveredItems.add(item.trim().toLowerCase());
  }
  for (const rows of Object.values(equivalenciasRows)) {
    for (const row of rows) coveredItems.add(row.insumoEquivalente.trim().toLowerCase());
  }

  for (const [key, info] of Object.entries(seed.info)) {
    if (!nextInfo[key]) {
      nextInfo[key] = info;
      changed = true;
    }
    if (!nextProdutos[key]) nextProdutos[key] = [];
    if (!nextEquivalencias[key]) nextEquivalencias[key] = [];
  }

  for (const [key, items] of Object.entries(seed.produtos)) {
    for (const item of items) {
      const lower = item.trim().toLowerCase();
      if (!lower || coveredItems.has(lower)) continue;
      nextProdutos[key] = [...(nextProdutos[key] ?? []), item];
      const seedRow = (seed.equivalencias[key] ?? []).find((row) => row.insumoEquivalente.trim().toLowerCase() === lower);
      if (seedRow) nextEquivalencias[key] = [...(nextEquivalencias[key] ?? []), seedRow];
      coveredItems.add(lower);
      changed = true;
    }
  }

  return { info: nextInfo, produtos: nextProdutos, equivalencias: nextEquivalencias, changed };
}
