...
async function shouldUseCompatSource(args: { req: NextRequest; supabase: ReturnType<typeof getSupabaseServerClient>; userId: string; isAdmin: boolean }) {
  ...
  const { data: state } = await args.supabase.from("fichas_tecnicas_state").select("payload").eq("id", stateId).maybeSingle();
  const payload = Array.isArray((state as any)?.payload) ? ((state as any).payload as any[]) : [];
  const legacyIngredients = payload.reduce(
    (total, recipe) => total + (Array.isArray(recipe?.ingredientRows) ? recipe.ingredientRows.length : Array.isArray(recipe?.ingredientes) ? recipe.ingredientes.length : 0),
    0,
  );
