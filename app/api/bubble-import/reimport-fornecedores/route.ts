...
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ only: ["fornecedores"], includeUnknown: true, prefix: runPrefix }),
      cache: "no-store",
    });
    ...
    const fornecedoresTouched = typeof parsed?.summary?.fornecedores === "number" ? parsed.summary.fornecedores : null;
    return json({ ok: true, runPrefix, fornecedoresTouched }, { status: 200 });
  } catch (err) {
