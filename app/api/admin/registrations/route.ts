...
export async function GET(req: NextRequest) {
  try {
    ...
    const [authUsers, mapResult, migrationsResult, companiesResult, membersResult] = await Promise.all([
      ...
      db.from("companies").select("id,fantasy_name,email"),
    ...
    const companyById = new Map<string, Company>(companies.map((company: any) => [String(company.id), { id: String(company.id), name: String(company.fantasy_name || "Sem nome") }]));
    ...
    const companiesByEmail = new Map<string, Company[]>(companies.filter((company: any) => company.email).map((company: any) => [String(company.email).trim().toLowerCase(), [{ id: String(company.id), name: String(company.fantasy_name || "Sem nome") }]]));
    ...
    for (const row of [...(mapResult.data ?? []), ...(migrationsResult.data ?? [])]) {
      ...
      legacyUsers.push({ email, authUserId: userId || null, companies: userCompanies, companiesCount: userCompanies.length, subscriptionMigrated: billingWasMigrated(migration), migrationStatus: String(migration?.validation_status || migration?.status || "") });
    }
