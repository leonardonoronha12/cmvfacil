...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!key) return json({ ok: false, error: "missing_key" }, { status: 400 });
    ...
    const path = `sessions/csv/${email}/${companyId}/progress/${key}.json`;
    const value = await downloadJsonFromStorage(supabase, bucket, path);
    if (!value) return json({ ok: false, error: "not_found" }, { status: 404 });
    return json({ ok: true, value }, { status: 200 });
  } catch (err) {
