...
export async function POST(req: Request) {
  ...
  const objectPath = `pre-preparo/${crypto.randomUUID()}.${ext}`;

  const upload = await supabase.storage.from("company-logos").upload(objectPath, file, { contentType, upsert: false });
  if (upload.error) return json({ error: "upload_failed", details: upload.error.message }, { status: 500 });
