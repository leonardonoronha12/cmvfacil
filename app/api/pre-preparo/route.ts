...
async function shouldUseCompatSource(args: { req: NextRequest; supabase: ReturnType<typeof getSupabaseServerClient>; userId: string; isAdmin: boolean }) {
  ...
  const enabled = String(process.env.BUBBLE_COMPAT_READ_PRE_PREPARO ?? "").trim().toLowerCase();
  if (!(enabled === "1" || enabled === "true" || enabled === "yes" || enabled === "on")) return false;

  const allowUsers = new Set(parseCsvEnv(process.env.BUBBLE_COMPAT_READ_PRE_PREPARO_USER_IDS).map((x) => x.toLowerCase()));
  const allowEmails = new Set(parseCsvEnv(process.env.BUBBLE_COMPAT_READ_PRE_PREPARO_EMAILS).map((x) => x.toLowerCase()));
