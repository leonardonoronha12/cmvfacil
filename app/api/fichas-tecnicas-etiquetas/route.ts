...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from("fichas_tecnicas_etiquetas_state").select("*").eq("id", id).maybeSingle();
...
export async function POST(req: NextRequest) {
  try {
    ...
    const supabase = getSupabaseServerClient(accessToken);
    const { error } = await supabase.from("fichas_tecnicas_etiquetas_state").upsert({ id, payload: rows } as any, { onConflict: "id" });
    if (error) return json({ error: error.message }, { status: 500 });
