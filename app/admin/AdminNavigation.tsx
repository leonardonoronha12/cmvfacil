...
export default function AdminNavigation(){const pathname=usePathname();const router=useRouter();const[pending,setPending]=useState("");useEffect(()=>{for(const[href]of[...monitoring,...tools])router.prefetch(href)},[router]);useEffect(()=>setPending(""),[pathname]);const link=([href,icon,label]:readonly [string,string,string])=><Link key={href} href={href} prefetch onClick={()=>setPending(href)} aria-current={(pending||pathname)===href?"page":undefined} className={(pending||pathname)===href?styles.active:""}><i>{icon}</i><span>{label}</span></Link>;return <aside className={styles.sidebar}><div className={styles.brand}><b><span>cmv</span>fácil</b><small>ADMIN</small></div><nav aria-label="Menu administrativo"><p>MONITORAMENTO</p>{monitoring.map(link)}<p>FERRAMENTAS</p>{tools.map(link)}</nav><div className={styles.sidebarFooter}><b>Painel global</b><span>Acesso restrito a administradores</span><Link href="/dashboard" prefetch>← Voltar ao sistema</Link></div></aside>}
