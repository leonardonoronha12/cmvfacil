...
export default function Page() {
  ...
  return (
    <main className="cmv-container">
      ...
      {savedAt ? <div className="cmv-alert cmv-alert-ok">Salvo com sucesso.</div> : null}
      ...
      <section className="cmv-grid">
        <div className="cmv-card">
          <form onSubmit={onSubmit}>
            ...
            <select value={messageTypeId} onChange={(e) => setMessageTypeId(e.target.value)} className="cmv-input" required>
              {MESSAGE_TYPES.map((t) => (
            ...
            <div className="cmv-help">Título do email. Ex.: Bem-vindo(a) ao CMV Fácil</div>
            <input value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} placeholder="Bem-vindo!" className="cmv-input" />

            <label className="cmv-label">Mensagem do Email</label>
            <div className="cmv-help">Corpo do email. Se você usa template na Brevo, pode usar esse texto como variável no template.</div>
            <textarea
              value={emailBody}
            ...
            <div className="cmv-help">Texto do WhatsApp. Dica: mensagens curtas e diretas costumam performar melhor.</div>
            <textarea
              value={whatsappBody}
          ...
          <div className="cmv-table-wrap">
            <table className="cmv-table">
              ...
              <tbody>
                {rows.length ? (
                  rows.map((r) => {
                    const active = r.event_type === messageTypeTrimmed;
                    return (
                      <tr key={r.event_type} onClick={() => setMessageTypeId(r.event_type)} className={`cmv-row ${active ? "cmv-row-active" : ""}`}>
                        <td className="cmv-td">
