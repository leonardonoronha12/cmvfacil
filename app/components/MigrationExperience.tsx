...
const steps: TourStep[] = [
  ...
  { path: "/dashboard", target: { selector: '[data-tour="cmv-start-date"]' }, passive: true, completion: { selector: '[data-tour="cmv-start-date"]', event: "change" }, icon: "📅", kicker: "Passo 1 — início", title: "Escolha a data inicial", text: "Selecione o inventário que representa o começo do período que deseja analisar.", improvement: "O estoque registrado nessa data será usado como estoque inicial do cálculo. Ao selecionar, o guia avança automaticamente.", bullets: ["Inventário inicial", "Começo do período", "Avanço automático"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-end-date"]' }, passive: true, completion: { selector: '[data-tour="cmv-end-date"]', event: "change" }, icon: "📅", kicker: "Passo 2 — encerramento", title: "Escolha a data final", text: "Selecione o inventário que encerra o período analisado.", improvement: "O estoque dessa data será comparado ao inicial, considerando as entradas realizadas entre as duas contagens.", bullets: ["Inventário final", "Fim do período", "Entradas consideradas"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-revenue"]' }, passive: true, completion: { selector: '[data-tour="cmv-revenue"]', event: "input", debounceMs: 700, requireValue: true }, icon: "💰", kicker: "Passo 3 — faturamento", title: "Informe o faturamento do período", text: "Digite o total vendido entre a data inicial e a data final.", improvement: "Esse valor permite transformar o custo das mercadorias em percentual do faturamento. Quando você parar de digitar, eu avanço.", bullets: ["Vendas do período", "Valor real", "CMV percentual"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-target"]' }, passive: true, icon: "🎯", kicker: "Passo 4 — meta opcional", title: "Defina sua meta de CMV", text: "A meta é o percentual máximo de custo que você deseja para a operação.", improvement: "Esse campo é opcional. Preencha para comparar o resultado real com a meta ou clique em Continuar para manter o valor atual.", bullets: ["Percentual desejado", "Comparação com o real", "Campo opcional"] },
  ...
  { path: "/dashboard", target: { selector: '[data-tour="cmv-result-card"]' }, passive: true, icon: "📊", kicker: "Resultado principal", title: "Leia o seu CMV Real", text: "Este percentual mostra quanto do faturamento foi consumido pelo custo das mercadorias no período selecionado.", improvement: "Compare o resultado com a meta da operação. Um percentual acima da meta indica que custos, preços, desperdícios ou rendimento precisam ser investigados.", bullets: ["Percentual do faturamento", "Compare com a meta", "Identifique desvios"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-comparison-card"]' }, passive: true, icon: "↕️", kicker: "Comparação e perdas", title: "Entenda a variação do resultado", text: "Este card compara o CMV com a meta e com o período anterior, além de mostrar o valor dos desperdícios registrado no intervalo.", improvement: "Use Ver variação de custo dos insumos para descobrir quais itens ficaram mais caros e pressionaram o resultado.", bullets: ["Meta versus realizado", "Período anterior", "Desperdícios"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-category-chart"]' }, passive: true, icon: "🍩", kicker: "Distribuição por categoria", title: "Veja onde o custo está concentrado", text: "O gráfico divide o CMV entre as categorias dos itens, como proteínas, bebidas, embalagens e pré-preparos.", improvement: "Passe o cursor sobre as fatias para conferir valores. Categorias desproporcionais ajudam a direcionar a análise e a negociação de compras.", bullets: ["Participação por categoria", "Valores no gráfico", "Priorize a investigação"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-flow-cards"]' }, passive: true, icon: "🧾", kicker: "Composição do período", title: "Confira os quatro cards do cálculo", text: "Estoque inicial é o valor existente no começo; Entradas são as compras; Estoque final é o saldo da última contagem; Saídas representam o consumo calculado no período.", improvement: "Esses cards permitem conferir rapidamente se as bases do cálculo fazem sentido antes de analisar item por item.", bullets: ["Estoque inicial", "Entradas e estoque final", "Saídas do período"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-items-table"]' }, passive: true, icon: "📋", kicker: "Detalhamento por item", title: "Destrinche o CMV na tabela", text: "Cada linha mostra estoque inicial, entradas, estoque final, saídas, custo médio e CMV daquele item.", improvement: "Use a busca e o filtro de categoria para localizar desvios. Clique no nome do item para abrir seu histórico de entradas, custos e movimentações.", bullets: ["Cálculo por item", "Busca e categoria", "Clique para ver o histórico"] },
  { path: "/dashboard", target: { selector: '[data-tour="cmv-table-item-history"]' }, icon: "🔎", kicker: "Auditoria do resultado", title: "Abra os detalhes e históricos", text: "Clique no nome do primeiro item destacado para conferir as entradas e os custos que formaram o resultado.", improvement: "O círculo verde indica exatamente onde clicar. O guia avança somente depois de abrir o item.", bullets: ["Clique no nome", "Histórico de entradas", "Rastreabilidade"] },
  { path: "/dashboard", target: { selector: '[data-tour="item-entry-history"]' }, passive: true, icon: "📚", kicker: "Histórico aberto", title: "Confira o histórico deste item", text: "Aqui você encontra as compras e movimentações que alteraram o estoque e o custo do item, com data, fornecedor, quantidade e preço.", improvement: "Use estes dados para investigar valores fora do esperado. Depois, clique em Continuar para seguir o tour.", bullets: ["Data e fornecedor", "Quantidade e preço", "Evolução do custo"] },
  ...
  { path: "/fornecedores", target: { text: "Novo Fornecedor", tag: "button" }, icon: "🚚", kicker: "Compras organizadas", title: "Gerencie fornecedores", text: "Aqui você mantém os fornecedores e os produtos vinculados a cada um.", improvement: "Clique em Novo Fornecedor para cadastrar empresa e contato. Os vínculos aceleram as notas e preservam o histórico de compras.", bullets: ["Clique em Novo Fornecedor", "Produtos vinculados", "Histórico preservado"] },
  ...
  { path: "/lista-de-compras", target: { selector: '[data-tour="shopping-period-range"]' }, alertSelectors: ['[data-tour="shopping-period-start"]', '[data-tour="shopping-period-end"]'], passive: true, icon: "📅", kicker: "Período da compra", title: "Entenda as duas datas", text: "Os dois campos destacados em amarelo formam o período: a primeira data representa o inventário inicial e a segunda o inventário final. A diferença entre elas ajuda o sistema a estimar o consumo diário de cada item.", improvement: "Nesta explicação, confira as duas datas destacadas e clique em Continuar. Use contagens completas e comparáveis; períodos inconsistentes produzem sugestões de compra pouco confiáveis.", bullets: ["Inventário inicial", "Inventário final", "Consumo diário"] },
  ...
  { path: "/lista-de-compras", target: { selector: '[data-tour="shopping-group-mode"]' }, alertSelectors: ['[data-tour="shopping-mode-category"]', '[data-tour="shopping-mode-supplier"]', '[data-tour="shopping-group-filter"]'], passive: true, icon: "🗂️", kicker: "Categoria e fornecedor", title: "Escolha como agrupar a compra", text: "Os controles destacados permitem trocar a organização da lista. No modo Categoria, você separa bebidas, proteínas e outros grupos. No modo Fornecedor, visualiza o que deve ser pedido de cada parceiro.", improvement: "O filtro abaixo acompanha o modo escolhido. No modo Fornecedor, a quantidade é convertida para a embalagem configurada no vínculo do parceiro. Confira os destaques e clique em Continuar.", bullets: ["Filtro por categoria", "Filtro por fornecedor", "Embalagem de compra"] },
  ...
  { path: "/lista-de-compras", target: { selector: '[data-tour="shopping-item-history"]' }, icon: "📚", kicker: "Histórico do item", title: "Abra o item para conferir a origem", text: "O primeiro nome da lista está destacado em verde. Clique nele para abrir seu histórico e entender entradas, custos e dados usados na sugestão.", improvement: "Confira o histórico quando uma quantidade sugerida parecer muito alta ou muito baixa.", bullets: ["Clique no item", "Histórico de entradas", "Audite a sugestão"] },
  { path: "/dashboard", target: { selector: '[data-tour="item-entry-history"]' }, passive: true, icon: "📚", kicker: "Histórico da compra", title: "Confira a origem da sugestão", text: "Aqui estão as entradas do item selecionado, com datas, fornecedores, quantidades, preços e subtotais.", improvement: "Use esses dados para entender o custo médio e conferir por que a lista sugeriu determinada quantidade. Depois, clique em Continuar.", bullets: ["Entradas do item", "Custo e fornecedor", "Rastreabilidade"] },
...
export default function MigrationExperience() {
  ...
  const goToStep = (nextStep: number) => {
    ...
    if (pathname !== destination) { setLocatedStep(-1); setPendingStep(bounded); router.prefetch(destination); router.push(destination); }
  ...
  return <>
    ...
    {tourOpen && step > 0 ? <aside data-cmv-tour-ui="true" key={step} style={coachStyle} className={`${styles.coach} ${continueOnlyStep ? styles.coachContinueOnly : ""} ${coachTargetsModal ? styles.coachModalCompact : ""}`} role="dialog" aria-label="Guia do novo CMV Fácil">
      ...
      <div className={styles.speech}>
        ...
        <h3>{isRouteTransition ? `Indo para ${steps[pendingStep!].path === "/insumos" ? "Insumos" : "a próxima etapa"}…` : steps[step].title}</h3>
        <p>{isRouteTransition ? "Aguarde só um instante. A próxima orientação aparecerá quando os controles estiverem prontos para usar." : typedText}<span className={styles.typingCursor} /></p>
