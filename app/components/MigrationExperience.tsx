  ...
  return <>
    ...
    {tourOpen && step > 0 ? <aside key={step} style={coachStyle} className={styles.coach} role="dialog" aria-label="Guia do novo CMV Fácil">
      ...
      <div className={styles.coachActions}>
        ...
        <div><button className={styles.coachBack} onClick={() => goToStep(step - 1)}>←</button>{steps[step].target ? <strong className={styles.clickHint}>Clique no destaque para continuar</strong> : <button className={styles.coachNext} onClick={() => advance(step)}>{activeModule && step >= moduleEndStep(activeModule) ? "Concluir módulo" : "Continuar"} <span>→</span></button>}</div>
      </div>
    </aside> : null}
