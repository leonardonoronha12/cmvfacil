...
export async function loadPrePreparoFromSupabase() {
  const res = await fetch(`/api/pre-preparo?ts=${Date.now()}`, { method: "GET", cache: "no-store" });
  ...
  return (Array.isArray(json.rows) ? (json.rows as any[]) : []) as PrePreparoStoreRow[];
}
