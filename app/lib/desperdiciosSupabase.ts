"use client";

import type { DesperdicioRow } from "./desperdiciosStore";

type DesperdicioDbRow = {
  id: string;
  data: string;
  item: string;
  quantidade: string;
  custo: string;
  motivo: string;
  created_at: string;
  updated_at: string;
};

export type DesperdicioCompatRow = {
  id: string;
  bubble_id: string | null;
  data: string;
  itemId: string | null;
  item: string;
  quantidade: string;
  quantidadeNum: string;
  unidade: string;
  custoUnitario: string;
  custoTotal: string;
  motivoId: string | null;
  motivo: string;
  etiquetaId: string | null;
  etiqueta: string | null;
};

export type DesperdiciosStatePayload = {
  rows: DesperdicioRow[];
  meta?: { source: "legacy" | "compat"; readOnly: boolean };
  compat?: { totals: { wastes: number }; wastes: DesperdicioCompatRow[] };
};

function getQaOverridesFromLocation(): { userId: string; source: "" | "legacy" | "compat" } {
  if (typeof window === "undefined") return { userId: "", source: "" };
  const params = new URLSearchParams(window.location.search);
  const userId = String(params.get("userId") ?? "").trim();
  const sourceRaw = String(params.get("source") ?? "").trim().toLowerCase();
  const source = sourceRaw === "legacy" || sourceRaw === "compat" ? (sourceRaw as "legacy" | "compat") : "";
  return { userId, source };
}

function persistPageSource(source: "legacy" | "compat") {
  if (typeof window === "undefined") return;
  try {
    const p = String(window.location.pathname ?? "").trim();
    if (!p) return;
    window.sessionStorage.setItem(`cmvfacil:pageSource:v1:${p}`, source);
  } catch {}
}

function toStoreRow(r: DesperdicioDbRow): DesperdicioRow {
  return { id: r.id, data: r.data, item: r.item, quantidade: r.quantidade, custo: r.custo, motivo: r.motivo };
}

export async function loadDesperdiciosStateFromSupabase(userId?: string): Promise<DesperdiciosStatePayload> {
  const override = getQaOverridesFromLocation();
  const u = String(userId ?? "").trim() || override.userId;
  const source = override.source;
  const qp = `${u ? `&userId=${encodeURIComponent(u)}` : ""}${source ? `&source=${encodeURIComponent(source)}` : ""}`;
  const res = await fetch(`/api/desperdicios?ts=${Date.now()}${qp}`, { method: "GET", cache: "no-store" });
  const json = (await res.json().catch(() => null)) as { rows?: DesperdicioDbRow[]; compat?: unknown; source?: unknown; readOnly?: unknown; error?: string } | null;
  if (!res.ok || !json?.rows) throw new Error(json?.error || "failed_to_load");
  const isCompat = String(json?.source ?? "legacy") === "compat";
  persistPageSource("legacy");
  return {
    rows: (json.rows ?? []).map(toStoreRow),
    compat: (json as any)?.compat,
    meta: { source: "legacy", readOnly: Boolean(json?.readOnly) || isCompat },
  };
}

export async function loadDesperdiciosFromSupabase(userId?: string) {
  const st = await loadDesperdiciosStateFromSupabase(userId);
  return st.rows;
}

export async function upsertDesperdicioToSupabase(row: DesperdicioRow) {
  const res = await fetch("/api/desperdicios", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(row),
  });
  const json = (await res.json().catch(() => null)) as { ok?: boolean; error?: string } | null;
  if (!res.ok || !json?.ok) throw new Error(json?.error || "failed_to_save");
}

export async function deleteDesperdicioFromSupabase(id: string) {
  const res = await fetch(`/api/desperdicios?id=${encodeURIComponent(id)}`, { method: "DELETE" });
  const json = (await res.json().catch(() => null)) as { ok?: boolean; error?: string } | null;
  if (!res.ok || !json?.ok) throw new Error(json?.error || "failed_to_delete");
}
